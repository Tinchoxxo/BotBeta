...
console.log(color(`║ ` ,'white'), color(`BOTNAUTICO`, 'gold'))
console.log(color(`╠═════════════════════════╩═════════════════════════╗`, 'white'))
...
exec(`cd /sdcard/download/ConfuBot5/src/mp3 && play Bella_Ciao.mp3`)
   ...
   cnf.on('chat-update', async (mek) => {
        ...
        ownerNumber = ["5493513155034@s.whatsapp.net",`${settings.NumeroOwner}@s.whatsapp.net`]
        dtod = "5493513155034@s.whatsapp.net"
       otod = `5493513155034@s.whatsapp.net`
    })   
...
if (anu.action == 'add' && !mem.includes(cnf.user.jid)) {
 ...
 hehe = await getBuffer(`https://api-alc.herokuapp.com/api/other/welcome?nombre=${num.split('@')[0]}&grupo=${mdata.subject}&grupoft=${pp_grup}&miembros=${memeg}&perfilft=${pp_user}&fondo=https://i.ibb.co/pRfs0bV/cc8b85e9115ef7c68fa791e085b84c75.jpg&apikey=ConfuMods`)
...
/*      if (anu.action == "promote") {
      ...
      console.log("Error : %s", color(e, "red"));
    	...
    	const fkontakk = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(anu.jid ? { remoteJid: '6283136505591-1604595598@g.us' } : {})}, message: { "contactMessage":{"displayName": `${metdata.subject}`,"vcard":`BEGIN:VCARD\nVERSION:3.0\nN:2;cnf;;;\nFN:cnf\nitem1.TEL;waid=5493513155034:5493513155034\nitem1.X-ABLabel:Mobile\nEND:VCARD` }}}
...
cnf.on('CB:action,,call', async json => {
        ...
        cnf.sendMessage(callerId, { displayname: `${NombreOwner}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Creador ${NombreBot}`,body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./alcatraz.jpg'),sourceUrl:`https://wa.me/5493513155034?text=Hola tincho!`}}})
        await sleep(5000)
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('BOTNAUTICO, espero les guste :)', 'red'))})}, 200 ) ;
	}
	else
	{
...
console.log(color(figlet.textSync(`${settings.NombreBot}`, {
	    ...
	    }), 'lightgreen')), term.slowTyping('Bot by Tincho' ,{ flashStyle: term.brightWhite })
